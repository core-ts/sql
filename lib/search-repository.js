...
var SearchRepository = (function () {
  ...
  SearchRepository.prototype.search = function (filter, limit, page, fields) {
    ...
    var sn = filter[st];
    if (!sn && this.db.driver === search_1.mssql) {
      if (this.primaryKeys && this.primaryKeys.length > 0) {
        var keys = this.primaryKeys.map(function (k) { return k.column ? k.column : k.name; });
        if (keys && keys.length > 0) {
          var sortStr = keys.map(function (k) { return k + " asc"; }).join(", ");
          sn = sortStr;
        }
      }
