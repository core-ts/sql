...
function buildFromQuery(query, sql, params, limit, page, mp, bools, provider, totalCol) {
  if (!limit || limit <= 0) {
    ...
    if (provider === exports.oracle || provider === exports.mssql) {
      if (!totalCol || totalCol.length === 0) {
        totalCol = "total";
      }
      var sql2 = buildPagingQueryForOracleOrMssql(sql, limit, offset, totalCol);
...
function buildPagingQueryForOracleOrMssql(sql, limit, offset, total) {
  if (!total || total.length === 0) {
    total = "total";
  }
  if (!offset) {
...
exports.buildPagingQueryForOracleOrMssql = buildPagingQueryForOracleOrMssql;
